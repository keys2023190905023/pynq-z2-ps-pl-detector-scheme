...
// Generic multi-channel feature-map stage for the PL-only fullhw path.
//
// Input layout on s_axis:
//   channel-major, raster scan within each channel
// Output layout on m_axis:
//   channel-major, raster scan within each output channel
//
// The stage keeps all scheduling inside PL:
// 1. load the full input tensor into on-chip RAM
// 2. fold input channels through the shared convolution engine
// 3. store packed output vectors on chip
// 4. stream them back out as channel-major bytes for the next PL stage
module TinyFullHwMultiChannelFeatureStage #(
    ...
    parameter integer STAGE_KIND       = 0,
    ...
    reg [31:0] stream_pixel_addr;
    reg [15:0] stream_channel;
    reg signed [7:0] stream_data;
    reg stream_valid;
    reg conv_ap_start;
    ...
    wire signed [OUT_CHANNELS*32-1:0] accum_rd_data;
    wire signed [OUT_CHANNELS*8-1:0] output_rd_data;

    wire conv_ap_done;
    wire conv_ap_idle;
    wire conv_accum_write_en;
    wire signed [OUT_CHANNELS*32-1:0] conv_accum_write_data;
    wire conv_ofm_valid;
    wire signed [OUT_CHANNELS*8-1:0] conv_ofm_write_data;
    wire conv_window_valid_dbg;

    reg [OUT_CHANNELS*8-1:0] output_wr_data;
    reg output_wr_en;
    reg [RAM_ADDR_WIDTH-1:0] output_wr_addr;

    integer idx;

    function signed [7:0] stage_center_coeff;
        input integer out_channel;
        input integer in_channel;
        begin
            case (STAGE_KIND)
                0: stage_center_coeff = (out_channel == in_channel) ? 8'sd1 : 8'sd0;
                1: stage_center_coeff = (((in_channel % 3) == out_channel) ? 8'sd1 : 8'sd0);
                default: stage_center_coeff = 8'sd0;
            endcase
        end
    endfunction

    function signed [31:0] stage_bias_value;
        input integer out_channel;
        begin
            stage_bias_value = 32'sd0;
        end
    endfunction

    function [OUT_CHANNELS*72-1:0] build_weight_tile;
        input integer in_channel;
        integer out_channel;
        reg signed [7:0] coeff;
        begin
            build_weight_tile = {OUT_CHANNELS*72{1'b0}};
            for (out_channel = 0; out_channel < OUT_CHANNELS; out_channel = out_channel + 1) begin
                coeff = stage_center_coeff(out_channel, in_channel);
    ...
    function [OUT_CHANNELS*32-1:0] build_bias_tile;
        input integer unused_select;
        integer out_channel;
        begin
            build_bias_tile = {OUT_CHANNELS*32{1'b0}};
            for (out_channel = 0; out_channel < OUT_CHANNELS; out_channel = out_channel + 1) begin
                build_bias_tile[out_channel*32 +: 32] = stage_bias_value(out_channel);
            end
        end
    endfunction

    wire [OUT_CHANNELS*72-1:0] weight_tile = build_weight_tile(current_channel);
    wire [OUT_CHANNELS*32-1:0] bias_tile = build_bias_tile(0);

    assign s_axis_tready = (state == ST_LOAD);
    assign m_axis_tdata = stream_data;
    assign m_axis_tvalid = (state == ST_STREAM) && stream_valid;
    assign m_axis_tlast =
        (state == ST_STREAM) &&
        stream_valid &&
        (stream_channel == OUT_CHANNELS - 1) &&
        (stream_pixel_addr == OUTPUT_PIXELS - 1);
    ...
    FeatureMapDualPortRam #(
        ...
        .rd_addr_a(stream_pixel_addr[RAM_ADDR_WIDTH-1:0]),
    ...
    always @(*) begin
        ...
        output_wr_data = {OUT_CHANNELS*8{1'b0}};
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            issue_count <= 32'd0;
            output_count <= 32'd0;
            stream_pixel_addr <= 32'd0;
            stream_channel <= 16'd0;
            stream_data <= 8'sd0;
            stream_valid <= 1'b0;
            conv_ap_start <= 1'b0;
        end else begin
            ...
            case (state)
                ...
                ST_STREAM: begin
                    if (!stream_valid) begin
                        stream_data <= output_rd_data[stream_channel*8 +: 8];
                        stream_valid <= 1'b1;
                    end else if (m_axis_tready) begin
                        if (stream_pixel_addr == OUTPUT_PIXELS - 1) begin
                            if (stream_channel == OUT_CHANNELS - 1) begin
                                ap_done <= 1'b1;
                                ap_idle <= 1'b1;
                                stream_valid <= 1'b0;
                                state <= ST_IDLE;
                            end else begin
                                stream_channel <= stream_channel + 1'b1;
                                stream_pixel_addr <= 32'd0;
                                stream_valid <= 1'b0;
                            end
                        end else begin
                            stream_pixel_addr <= stream_pixel_addr + 1'b1;
                            stream_valid <= 1'b0;
                        end
                    end
                end
