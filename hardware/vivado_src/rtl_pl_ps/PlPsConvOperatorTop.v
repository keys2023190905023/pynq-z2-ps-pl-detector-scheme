...
module PlPsConvOperatorTop #(
    ...
    reg busy;
    reg ofm_pipe_valid;
    reg ofm_pipe_frame_last;
    reg signed [OUT_CHANNELS_PAR*32-1:0] ofm_pipe_partial_sums;
    integer idx;
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            ofm_pipe_valid  <= 1'b0;
            ofm_pipe_frame_last <= 1'b0;
            ofm_pipe_partial_sums <= {OUT_CHANNELS_PAR{32'sd0}};
        end else begin
            ap_done        <= 1'b0;
            accum_write_en <= 1'b0;
            ofm_valid      <= 1'b0;
            ...
            if (ofm_pipe_valid) begin
                for (idx = 0; idx < OUT_CHANNELS_PAR; idx = idx + 1) begin
                    partial_value = ofm_pipe_partial_sums[idx*32 +: 32];
            ...
            if (busy && pixel_valid && window_valid) begin
                if (channel_last) begin
                    ofm_pipe_valid <= 1'b1;
                    ofm_pipe_frame_last <= frame_last;
                    ofm_pipe_partial_sums <= next_partial_sums;
                end else begin
                    accum_write_en   <= 1'b1;
